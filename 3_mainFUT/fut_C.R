...
for (per in pers){
cat(paste('Ready for', scenar_lulc, per, toupper(model_name),  'future REG predictions of', ispi_name, ' using the ',nesting_method,' method for scale-nesting ...\n'))
...
cov_info_pres<-cov_info_pres[!duplicated(cov_info_pres$ID, fromLast=T),]
lr_pres_ID<-cov_info_pres$ID

# D.1 List available future layers for cov_ID
cov_info_fut<-cov_info[cov_info$ID %in% cov_ID & cov_info$period=="future" & cov_info$scenario==scenar_lulc & cov_info$start_year==gsub("_.*", "", per) & cov_info$end_year==gsub(".*_", "", per),]
...
if("mainGLO" %in% cov){
glo_out<-list.files(paste0(scr_path,"/outputs/",project,"/d15_ensembles-fut/glo/",scenar_clim,"/",per,"/",ispi_name), pattern=".rds", full.names = TRUE)
...
nsdm.savethis(object=list(ndata_bck=ndata_bck, template=template, nona_ix=hab_df_reg$covdf_ix),
              ...
              save_path=paste0(scr_path,"/outputs/",project,"/d13_preds-fut/reg/",nesting_method,"/",scenar_lulc,"/",per))
}
}
